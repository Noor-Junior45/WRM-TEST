...
export const POS: React.FC = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white md:bg-gray-50 md:p-6 pb-32 animate-in fade-in relative">
      ...
      <Modal isOpen={showCheckout} onClose={() => { setShowCheckout(false); window.history.back(); }} title="Payment" className="!max-w-lg bg-white border-2 border-slate-300 shadow-2xl !p-6"><div className="text-center pb-2"><div className="bg-slate-100 border-2 border-slate-200 rounded-2xl p-5 mb-5 text-center"><span className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1">Total Amount Due</span><h2 className="text-4xl font-black text-slate-900">₹{totals.net.toFixed(2)}</h2></div><div className={`bg-slate-50 rounded-2xl p-4 text-left space-y-3 mb-5 border-2 ${showCheckoutWarning ? 'border-red-400 bg-red-50/50' : 'border-slate-200'} ${shakeError ? 'shake-element' : ''}`}>{selectedCustomer ? <div className="flex justify-between items-center text-sm font-bold text-slate-800"><span className="flex items-center gap-1.5 text-slate-900 text-base"><User size={16} className="text-indigo-600 shrink-0" />{selectedCustomer.name} {selectedCustomer.isWholesaler && <Star size={12} className="text-amber-500 fill-amber-500 shrink-0"/>}</span><span className="text-slate-500 text-sm font-mono">{selectedCustomer.phone}</span></div> : <div className="space-y-3"><span className="text-[10px] font-black uppercase text-slate-400 tracking-wider block mb-1">Quick Customer Details</span><div className="relative"><User size={14} className="absolute left-3 top-3.5 text-slate-400" /><input ref={quickNameRef} className="w-full pl-9 pr-3 py-2.5 text-sm bg-white border-2 border-slate-200 rounded-xl focus:border-indigo-500 outline-none font-bold text-slate-800 transition-all placeholder:text-slate-400" placeholder="Customer Name" value={quickCustName} onChange={(e) => setQuickCustName(e.target.value)} /></div><div className="relative"><Phone size={14} className="absolute left-3 top-3.5 text-slate-400" /><input className="w-full pl-9 pr-3 py-2.5 text-sm bg-white border-2 border-slate-200 rounded-xl focus:border-indigo-500 outline-none font-bold text-slate-800 transition-all placeholder:text-slate-400" placeholder="Phone Number" value={quickCustPhone} onChange={(e) => setQuickCustPhone(e.target.value.replace(/\D/g, ''))} maxLength={10} /></div></div>}</div><div className="mb-6"><div className="bg-slate-50 p-4 rounded-2xl border-2 border-slate-200 text-left mb-5"><label className="text-xs font-black text-slate-700 uppercase tracking-widest block mb-2">Paying Now</label><div className="flex items-center relative"><span className="absolute left-3 text-2xl font-black text-slate-400">₹</span><input type="number" className="w-full pl-9 pr-3 py-2.5 text-3xl font-black bg-white border-2 border-slate-200 rounded-xl outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all text-slate-900" value={partialPaidAmount} onChange={(e) => setPartialPaidAmount(e.target.value)} /></div></div><div className="bg-slate-50 p-4 rounded-2xl border-2 border-slate-200 text-left mb-6"><label className="text-xs font-black text-slate-700 uppercase tracking-widest block mb-2">Payment Method</label><div className="grid grid-cols-4 gap-2">{[{ id: 'Cash', icon: Banknote }, { id: 'UPI', icon: Smartphone }, { id: 'Card', icon: CreditCard }, { id: 'Pay Later', icon: Clock }].map(method => { const isSelected = paymentMethod === method.id; return (<button key={method.id} onClick={() => handlePaymentMethodClick(method.id as PaymentMethod)} className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all ${isSelected ? 'bg-slate-900 border-slate-900 text-white shadow-sm' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100 hover:border-slate-300'}`}><method.icon size={20} className={`mb-1.5 ${isSelected ? 'text-white' : 'text-slate-500'}`}/><span className="text-[11px] font-black uppercase tracking-wider">{method.id}</span></button>); })}</div></div></div><div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5"><Button onClick={() => handleCheckout('print')} className="py-3 px-4 font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md flex items-center justify-center gap-2 transition-all active:scale-95"><Printer size={18}/><span>Print Bill</span></Button><Button onClick={() => handleCheckout('share')} className="py-3 px-4 font-bold bg-[#25D366] hover:bg-[#20ba56] text-white rounded-xl shadow-md flex items-center justify-center gap-2 transition-all active:scale-95"><WhatsAppLogo /><span>WhatsApp</span></Button><Button onClick={() => handleCheckout('save')} className="py-3 px-4 font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-md flex items-center justify-center gap-2 transition-all active:scale-95"><Save size={18}/><span>Save Only</span></Button></div></div></Modal>
